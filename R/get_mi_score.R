...
#' @param score_in_list_format optional; if \code{FALSE} (default), returns a long-format data frame with one score per subject per endpoint; if \code{TRUE}, returns the study-level list/data frame \code{mi_score_final_list_df} as before.
#' @param xpt_dir Optional; path to a directory containing XPT files for one study (flat: xpt_dir/mi.xpt, etc.).
#' @return When \code{score_in_list_format} is \code{FALSE}, a data frame with columns \code{STUDYID}, \code{USUBJID}, \code{endpoint}, \code{score} (one row per subject per microscopical finding). When \code{TRUE}, the study-level summary (or \code{calculated_mi_score} in legacy code paths).
#'
#' @examples
#' \dontrun{
