...
#' @return When \code{score_in_list_format} is \code{FALSE}, a data frame with columns \code{STUDYID}, \code{USUBJID}, \code{endpoint}, \code{score} (one row per subject per liver test, e.g. SERUM | ALT). When \code{TRUE}, a per-subject wide data frame with columns \code{STUDYID}, \code{USUBJID}, \code{ARMCD}, \code{alt_zscore}, \code{ast_zscore}, \code{alp_zscore}, \code{ggt_zscore}, \code{bili_zscore}, \code{alb_zscore}.
#'
#' @examples
#' \dontrun{
...
lb_per_subject_endpoint <- dplyr::bind_rows(
  zscore_serum_alt %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = alt_zscore),
  zscore_serum_ast %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = ast_zscore),
  zscore_serum_alp %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = alp_zscore),
  zscore_serum_ggt %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = ggt_zscore),
  zscore_serum_bili %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = bili_zscore),
  zscore_serum_alb %>% dplyr::select(STUDYID, USUBJID, endpoint = LBTESTCD, score = alb_zscore)
)
