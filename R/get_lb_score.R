...
#' @param score_in_list_format optional; if \code{FALSE} (default), returns a long-format data frame with one score per subject per endpoint; if \code{TRUE}, returns the study-level table \code{master_lb_scores} as before.
#' @param xpt_dir Optional; path to a directory containing XPT files for one study (flat: xpt_dir/lb.xpt, etc.).
#' @return When \code{score_in_list_format} is \code{FALSE}, a data frame with columns \code{STUDYID}, \code{USUBJID}, \code{endpoint}, \code{score} (one row per subject per liver test, e.g. SERUM | ALT). When \code{TRUE}, the study-level table with \code{avg_*_zscore} columns.
#'
#' @examples
#' \dontrun{
