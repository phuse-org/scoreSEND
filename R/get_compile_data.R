#' @title Build subject compile data (TK removal and optional terminal SETCDs)
...
#' @param terminal_setcds_only If \code{TRUE} (default), after TK cleaning, keep only subjects whose \code{SETCD} is in \code{treatment_group} from \code{\link{get_treatment_group}} (terminal-sacrifice cohorts). If \code{FALSE}, all non-TK DM subjects are kept (no SETCD filter; broader cohort). Ignored when \code{fake_study} is \code{TRUE}.
#' @return dataframe
#'
#' @examples
#' \dontrun{
...
# For each pool ID in TKPools, retrieve corresponding rows from pooldef table
 ...
 cleaned_CompileData <- CompileData[
   !(CompileData$USUBJID %in% tK_animals_df$USUBJID),]
    ...
    # Step-3 (optional): restrict to terminal-sacrifice SETCDs via get_treatment_group
    if (isTRUE(terminal_setcds_only)) {
      ...
      if (length(terminal_setcds) == 0L) {
        warning("get_compile_data: treatment_group is empty; no subjects retained when terminal_setcds_only = TRUE.",
                call. = FALSE)
      }
